...
function updatestatus(){
	...
	xmlhttp.onreadystatechange = function(){
  	if(xmlhttp.status == 200 && xmlhttp.readyState == 4){
  			cur_ = xmlhttp.responseText;
    		cur_status = parseInt(cur_);
  		}
	};
	xmlhttp.open("GET", "alert/getcur.php", true);
...
function clicked(){
	...
	if (pre_status == 0 && cur_status == 1){
		...
		//swal("haha");
		document.getElementById("popstatus").innerHTML = "pop up on <button onclick = 'closepop()'>close</button>";	
		changestatus();
...
function closepop(){
	...
	pre_status = 0;
	changestatus();
	document.getElementById("popstatus").innerHTML = "pop up closed";
}
